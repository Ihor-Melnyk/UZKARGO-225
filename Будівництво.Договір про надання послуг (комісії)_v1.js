...
function setAdditionalSignatory() {
  var attrOrgAgentSurname2 = EdocsApi.getAttributeValue("OrgAgentSurname2");
  if (attrOrgAgentSurname2.value) {
    var OrganizationId = EdocsApi.getAttributeValue("OrganizationId").value;
    if (OrganizationId) {
      var data = EdocsApi.getContractorData(OrganizationId);
      if (data) {
...
function onTaskExecuteSendOutDoc(routeStage) {
  ...
  var methodData = {
    ExtSysDocVersion: CurrentDocument.version,
    extSysDocId: EdocsApi.getAttributeValue("DocId")?.value,
  };
